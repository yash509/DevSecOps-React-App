pipeline {
    ...
    stages {
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag react-app yash5090/react-app:latest " 
                        //}
                    }
                }
            }
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/react-app:latest "
                        //}
                    }
                }
            }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/react-app:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Deploy to container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name react-app -p 3000:3000 yash5090/react-app:latest' 
                //}
            }
        }
